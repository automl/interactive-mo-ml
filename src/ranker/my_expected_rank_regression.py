...
from csrank.objectranking.util import complete_linear_regression_dataset
...
class MyExpectedRankRegression(ObjectRanker, Learner):
    def __init__(
        self,
        alpha=0.0,
        l1_ratio=0.5,
        ...
        **kwargs,
        ...
        elastic net (when ``l1_ratio > 0``). The target for an object :math:`x_k \\in Q` is :math:`r(x_k) = \\frac{\\pi(k)}{n} \\quad`, where :math:`\\pi(k)` is the rank of the :math:`x_k`. The regression model learns a function
        :math:`F \\colon \\mathcal{X} \\to \\mathbb{R}`. The ranking for the given query set :math:`Q` defined as :math:`ρ(Q)  = \\operatorname{argsort}_{x \\in Q}  \\; -1*F(x)`
